...
Stack: [LENGUAJE + FRAMEWORK + DB - detectado o sugerido. REGLAS ESTRICTAS DE BD: Si el proyecto es un landing page, VSL (Video Sales Letter), funnel, pagina de ventas, sitio estatico o portfolio → NO incluir base de datos, OMITIR BD completamente. Si el proyecto es una app movil (Flutter, React Native, Swift, Kotlin, Dart, iOS, Android) SIEMPRE recomendar Firebase. Para el resto de proyectos (web, API, backend, scripts, CLI, desktop, etc.) SIEMPRE recomendar Supabase. ESTA PROHIBIDO recomendar Firebase para proyectos que no sean apps moviles.]
