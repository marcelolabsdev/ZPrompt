...
Stack: [LENGUAJE + FRAMEWORK + DB - detectado o sugerido. PREFERENCIA DE LENGUAJE: Siempre preferir TypeScript por sobre JavaScript. REGLAS ESTRICTAS DE BD: Si el proyecto es un landing page, VSL (Video Sales Letter), funnel, pagina de ventas, sitio estatico o portfolio → OMITIR BD completamente del Stack sin mencionar ni hacer referencia a bases de datos en ningun lugar del prompt. NO escribir "sin base de datos" ni nada similar. Si el proyecto es una app movil (Flutter, React Native, Swift, Kotlin, Dart, iOS, Android) SIEMPRE recomendar Firebase. Para el resto de proyectos (web, API, backend, scripts, CLI, desktop, etc.) SIEMPRE recomendar Supabase. ESTA PROHIBIDO recomendar Firebase para proyectos que no sean apps moviles.]
Deploy: Vercel (siempre incluir esta linea para proyectos web, landing pages, VSLs, funnels, paginas de ventas, APIs o backends. NO incluir esta linea para apps moviles/celulares. No escribir "Deploy: OMITIDO" ni nada similar, simplemente omitir la linea completa.)
